<template>
  ...
  <header :class="[
    ...
    <!-- Replace the existing logo div in PRIMARY nav -->
  ...
  <header :class="[
    ...
    <!-- Logo -->
    <!-- <div class="text-3xl font-bold bg-clip-text text-transparent" :style="{ backgroundImage: 'var(--logo-gradient)' }">
      Dev <span class="font-black">Portfolio</span>
    </div> -->
    <!-- Replace the existing logo div in STICKY nav -->
    ...
    <nav class="flex items-center">
      <ul class="flex flex-row space-x-5">
        <li v-for="item in Menu" :key="item.name" class="group relative">
          <a :href="item.href" class="relative block transition-colors duration-300 text-sm font-medium tracking-wide"
            :style="{ color: activeSection === item.href ? 'var(--accent-text)' : '' }"
            :class="activeSection !== item.href ? 'text-gray-400' : ''"
            @mouseenter="e => { if (activeSection !== item.href) e.currentTarget.style.color = 'var(--accent-text)' }"
            @mouseleave="e => { if (activeSection !== item.href) e.currentTarget.style.color = '' }"
...
// Map color scheme names to logo filenames
const logoSchemeMap = {
