      ...
      <div class="grid lg:grid-cols-2 gap-10 xl:gap-14 items-center">
        ...
        <div class="lg:h-full flex justify-center lg:justify-start order-2 lg:order-1">
          <div class="relative w-full max-w-md">
            <div data-aos="zoom-in" class="p-4 relative rounded-3xl backdrop-blur-sm shadow-2xl accent-card">
              <div class="grid grid-cols-2 gap-4">
                <div v-for="image in aboutImage" :key="image.id" class="relative group" :data-aos="image.animation"
                  :data-aos-delay="image.delay">
                  <div class="rounded-2xl p-1 border shadow-lg"
                    :style="{ background: image.gradient, borderColor: 'transparent' }">
              ...
              <div
                ...
                :style="{ background: 'var(--cta-gradient)', borderColor: 'var(--accent-border)' }" data-aos="fade-up"
                data-aos-delay="600">
        ...
        <div class="text-center lg:text-left" data-aos="fade-left">
          ...
          <h2 class="font-bold text-4xl md:text-5xl lg:text-6xl leading-loose"
            :style="{ color: 'var(--surface-text)' }">
            {{ t('about_') }}<span class="inline-block py-2 text-transparent bg-clip-text"
              :style="{ backgroundImage: 'var(--logo-gradient)' }">{{ t('_me') }}</span>
          </h2>
        ...
        <p class="pt-6 text-center lg:text-left text-lg leading-relaxed" :style="{ color: 'var(--surface-text-sub)' }"
        ...
        <div class="order-1 lg:order-2" data-aos="fade-left">
          ...
          <div class="grid sm:grid-cols-2 gap-4 mb-10">
            <div v-for="skill in skills" :key="skill.id"
              class="flex items-start space-x-3 p-3 rounded-xl border transition-all duration-300" :style="{
                backgroundColor: 'var(--surface-card)',
                borderColor: 'var(--surface-border)'
              }">
              <component :is="skill.icon" class="w-6 h-6 shrink-0" :style="{ color: skill.iconColor }" />
              <div>
                <h4 class="font-medium text-sm" :style="{ color: 'var(--surface-text)' }">{{ skill.title }}</h4>
                <p class="text-xs" :style="{ color: 'var(--surface-text-sub)' }">{{ skill.desc }}</p>
              </div>
            </div>
          </div>
          ...
          <div class="flex flex-wrap gap-4">
            <component v-for="btn in ctaButtons" :key="btn.id" :is="btn.type === 'router' ? 'router-link' : 'a'"
              ...
              class="group px-6 py-3 rounded-full flex items-center font-semibold transition-all duration-300 cursor-pointer"
              :style="btn.style" @mouseenter="e => btn.onHover && btn.onHover(e)"
              @mouseleave="e => btn.onLeave && btn.onLeave(e)">
              <component v-if="btn.iconLeft" :is="btn.iconLeft"
                class="w-5 h-5 mr-2 group-hover:translate-y-1 transition-transform" />
              {{ t(btn.text) }}
              <component v-if="btn.iconRight" :is="btn.iconRight"
...
const aboutImage = [
  {
    ...
    badge: 'Professional', badgeColor: 'var(--accent-mid-dark)',
  ...
  },
  {
    ...
    badge: 'Creative', badgeColor: 'var(--accent-to-mid)',
    animation: 'fade-left', delay: 300,
  },
  {
    id: 3, src: about3, alt: 'Developer',
    gradient: 'linear-gradient(to right, var(--accent-mid), var(--accent-to-mid))',
    badge: 'Developer', badgeColor: 'var(--accent-mid-dark)',
  ...
  },
  {
    ...
    badge: 'Design', badgeColor: 'var(--accent-to-dark)',
...
const ctaButtons = [
  {
    id: 1, text: 'view_portfolio', iconRight: ArrowRightIcon,
    ...
    to: '#projects', type: 'anchor',
  },
  {
    id: 2, text: 'contact_me', iconLeft: ChatBubbleLeftRightIcon,
    ...
    to: '#contact', type: 'anchor',
  },
  {
    id: 3, text: 'see_more', iconRight: ArrowRightIcon,
    style: { border: '1px solid var(--accent-text)', color: 'var(--accent-text)' },
    onHover: e => e.currentTarget.style.backgroundColor = 'var(--accent-bg)',
    onLeave: e => e.currentTarget.style.backgroundColor = 'transparent',
    to: '/details', type: 'router',
  },
];
</script>
