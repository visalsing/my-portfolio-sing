        ...
        <div class="mb-12 space-y-6" data-aos="fade-up">
          <div class="flex flex-col md:flex-row gap-4 items-center justify-between">
            ...
            <div class="relative w-full md:w-96">
              ...
              <input v-model="searchQuery" type="text" placeholder="Search project name..."
            ...
            <div v-if="showFilters" class="filter-panel grid grid-cols-1 md:grid-cols-2 gap-8 p-6 rounded-2xl">
              <div>
                ...
                <div class="flex flex-wrap gap-2">
                  <button v-for="cat in techStack" :key="cat" @click="selectedFilter = cat.toLowerCase()"
                    class="filter-chip px-3 py-1.5 rounded-md text-xs transition-all"
                    :class="selectedFilter === cat.toLowerCase() ? 'filter-chip-active' : 'filter-chip-idle'">{{ cat
                    }}</button>
                </div>
              </div>
              <div>
                ...
                <div class="flex flex-wrap gap-2">
                  <button v-for="tType in projectTypes" :key="tType" @click="selectedType = tType.toLowerCase()"
                    class="filter-chip px-3 py-1.5 rounded-md text-xs transition-all"
                    :class="selectedType === tType.toLowerCase() ? 'filter-chip-active' : 'filter-chip-idle'">{{ tType
                    }}</button>
                </div>
              </div>
            </div>
        ...
        <div v-if="filteredProjects.length > 0">
          <div v-for="group in groupedProjects" :key="group.name" class="mb-12">
            ...
            <div v-if="isGrouped" class="flex items-center gap-4 mb-8 mt-12" data-aos="fade-right">
              <h3 class="text-xl font-bold bg-clip-text text-transparent whitespace-nowrap"
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
              <div v-for="(project, index) in group.projects" :key="project.id"
                ...
                <div class="relative overflow-hidden">
                  ...
                  <div class="absolute top-4 right-4">
                    <span class="px-3 py-1 text-white text-[10px] rounded-full font-bold uppercase tracking-wider"
                ...
                <div class="p-6">
                  ...
                  <div class="flex flex-wrap gap-2 mb-4">
                    <span v-for="tech in project.technolg" :key="tech.name" class="px-2 py-1 border rounded text-[10px]"
                  ...
                  <div class="flex gap-4">
                    <template v-for="link in project.links" :key="link.name">
                      <router-link v-if="link.name === 'View'" :to="{ name: 'EachProject', params: { id: project.id } }"
...
// ── Project data ──
// Utility helper to safely map image names into hosted URLs
// Note: Adjusted back to two directory steps '../../assets/...' based on your relative path
...
const projects = ref([
  {
    ...
    image: getImageUrl('admin-dashboard.png'),
    category: 'Web',
    filters: ['web'],
    ...
    technolg: [
      ...
      { name: "Chart.js", useAccent2: false },
    ],
    links: [
  ...
  },
  {
    ...
    desc: 'Lorem Ipsum is simply dummy text of the printing and typesetting industry.',
    image: getImageUrl('portfolio-web.png'),
    category: 'Web',
    filters: ['web'],
    type: 'personal project',
    technolg: [
      { name: "Vue.js", useAccent2: true },
      { name: "Tailwind CSS", useAccent2: false },
      { name: "Vite.js", useAccent2: true },
      { name: "HTML", useAccent2: false },
      { name: "CSS", useAccent2: true },
      { name: "JavaScript", useAccent2: false },
      { name: "EmailJS", useAccent2: true },
      { name: "Telegram Bot", useAccent2: false },
      { name: "Python", useAccent2: true },
      { name: "Git", useAccent2: false },
      { name: "GitHub", useAccent2: true },
      { name: "Vercel", useAccent2: false },
      { name: "Claude", useAccent2: true },
      { name: "Gemini", useAccent2: false },
    ],
    links: [
  ...
  },
  {
    ...
    desc: `Lorem Ipsum is simply dummy text of the printing and typesetting industry.`,
    image: getImageUrl('spider15.png'),
    category: 'Web',
    filters: ['web'],
    type: 'team project',
    technolg: [
      ...
      { name: "Loyalty Programme", useAccent2: true },
      { name: "PlasGate", useAccent2: false },
    ],
    links: [
      { name: "Live Demo", url: "https://my-portfolio-sing.vercel.app/", icon: EyeIcon, useAccent2: false },
      { name: "Code", url: "https://github.com/visalsing/my-portfolio-sing", icon: CodeBracketIcon, useAccent2: true },
